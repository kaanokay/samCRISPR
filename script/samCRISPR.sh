...
while [[ "$#" -gt 0 ]]; do
	...
	--bam)
		bam_list="$2"
		shift
		;; # Expecting a file containing paths of BAM files
	--quantification-window)
		quantification_window="$2"
		shift
		;;
	*)
...
while read -r bam_file; do
	...
	while read -r chr start end strand name; do
		...
		# Ensure a true 0% prints as "0", not blank:
if [ -z "$Knockout_efficiency" ]; then
    Knockout_efficiency="0"
fi
...
")
		...
		<"$sgRNA" # End of inner loop for sgRNA processing
...
script_end=$(date +%s)
# Compute and report total elapsed seconds
total_elapsed=$(( script_end - script_start ))
echo "process time: ${total_elapsed}s"
